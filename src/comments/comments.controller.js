...
async function list(req, res, next) {
  const listIt = await service.list();
  res.json({ data: listIt });
}

async function listCommenterCount(req, res, next) {
  const listCount = await service.listCommenterCount();
  const mapped = listCount.map((oneItem) => {
    return {
      commenter_email: oneItem.commenter_email,
      count: Number(oneItem.count),
    };
  });
